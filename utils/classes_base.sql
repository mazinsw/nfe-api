...
CREATE TABLE IF NOT EXISTS `NF` (
  ...
  `emitente` VARCHAR(45) NOT NULL DEFAULT 'new Emitente()' COMMENT '[F:new Emitente()][S]',
  `cliente` VARCHAR(45) NOT NULL DEFAULT 'new Cliente()' COMMENT '[F:new Cliente()][S]',
  `produtos` VARCHAR(45) NOT NULL DEFAULT 'array()' COMMENT '[F:array()][S:S][U:produtos|produto]',
  `transporte` VARCHAR(45) NOT NULL DEFAULT 'new Transporte()' COMMENT '[F:new Transporte()][S]',
  `pagamentos` VARCHAR(45) NOT NULL DEFAULT 'array()' COMMENT '[F:array()][S:S][U:pagamentos|pagamento]',
...
CREATE TABLE IF NOT EXISTS `Imposto` (
  ...
  `grupo` ENUM('icms', 'pis', 'cofins', 'ipi', 'ii', 'pisst') NOT NULL COMMENT 'Grupo do imposto[E:ICMS|PIS|COFINS|IPI|II|PISST]',
...
CREATE TABLE IF NOT EXISTS `Imposto.ICMS.Cobranca` (
  `normal` VARCHAR(25) NOT NULL DEFAULT 'new Normal()' COMMENT '[D][F:new Normal()][S]')
ENGINE = InnoDB
...
-- -----------------------------------------------------
-- Table `Imposto.ICMS.Isenta`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Imposto.ICMS.Isenta` ;

CREATE TABLE IF NOT EXISTS `Imposto.ICMS.Isenta` (
...
CREATE TABLE IF NOT EXISTS `Imposto.ICMS.Generico` (
  `id` INT NOT NULL)
ENGINE = InnoDB
COMMENT = 'Tributação pelo ICMS\n90 - Outras, estende de Normal[L:ICMS90][K:Imposto\\ICMS|Imposto\\ICMS\\][H:Normal]';
...
CREATE TABLE IF NOT EXISTS `Evento` (
  ...
  `on_forma_emissao` VARCHAR(45) NOT NULL COMMENT 'Chamado quando a forma de emissão da nota fiscal muda para normal ou contigência[S]',
  `on_nota_enviada` VARCHAR(45) NOT NULL COMMENT 'Chamado quando a nota foi enviada e aceita pela SEFAZ (Não é chamado quando em contigência)[S]',
...
CREATE TABLE IF NOT EXISTS `Ajuste` (
  ...
  `pasta_xml_final` VARCHAR(45) NOT NULL COMMENT 'Pasta onde fica os XML das notas após serem enviadas e aceitas pela SEFAZ[S]',
  `pasta_xml_gerado` VARCHAR(45) NOT NULL COMMENT 'Pasta onde ficam os XMLs após gerado e antes de serem aceitos[S]',
  `token` VARCHAR(45) NOT NULL COMMENT '[S]',
  `csc` VARCHAR(45) NOT NULL COMMENT '[S]')
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `Imposto.PISST.Aliquota` (
  `id` INT NOT NULL)
ENGINE = InnoDB
COMMENT = 'Este grupo só deve ser informado se o produto for sujeito a PIS por ST, CST = 05, a informação deste grupo não desobriga a informação do grupo PIS.[L:PISST][K:Imposto\\PISST|Imposto\\PISST\\][H:Imposto\\PIS\\Aliquota]';
...
CREATE TABLE IF NOT EXISTS `Imposto.PISST.Quantidade` (
  `id` INT NOT NULL)
ENGINE = InnoDB
COMMENT = 'Quantidade Vendida x Alíquota por Unidade de Produto[L:PISST][K:Imposto\\PISST|Imposto\\PISST\\][H:Imposto\\PIS\\Quantidade]';
